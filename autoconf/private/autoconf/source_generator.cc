...
std::string replace_undef(const std::string& content,
    ...
    output.append(searchStart, content.cend());
...
void SourceGenerator::generate_config_header(
    ...
    std::string content = process_template(templateContent);
...
std::string SourceGenerator::process_template(
    ...
    if (!config_.package_name.empty()) {
        ...
        std::regex packageNameRegex("@PACKAGE_NAME@");
        content =
            std::regex_replace(content, packageNameRegex, config_.package_name);
    ...
    if (!config_.package_version.empty()) {
        ...
        std::regex packageVersionRegex("@PACKAGE_VERSION@");
        content = std::regex_replace(content, packageVersionRegex,
