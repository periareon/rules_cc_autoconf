...
CheckResult CheckRunner::run_check(const Check& check) {
    switch (check.type()) {
        ...
        default:
            DebugLogger::warn("Unknown check type");
...
CheckResult CheckRunner::check_sizeof(const Check& check) {
    if (!check.code().has_value()) {
        DebugLogger::warn("sizeof check missing code");
        return CheckResult(check.define(), "0", false);
    }

    std::string code = *check.code();

    std::optional<int> size = try_compile_and_run(code, check.language());
    if (size.has_value()) {
...
CheckResult CheckRunner::check_alignof(const Check& check) {
    if (!check.code().has_value()) {
        DebugLogger::warn("alignof check missing code");
        return CheckResult(check.define(), "0", false);
    }

    std::string code = *check.code();

    std::optional<int> alignment = try_compile_and_run(code, check.language());
...
CheckResult CheckRunner::check_decl(const Check& check) {
    if (!check.code().has_value()) {
        DebugLogger::warn("decl check missing code");
        return CheckResult(check.define(), "0",
                           true);  // Always success, value indicates result
    }
...
CheckResult CheckRunner::check_member(const Check& check) {
    if (!check.code().has_value()) {
        DebugLogger::warn("member check missing code");
        return CheckResult(check.define(), "0", false);
    }
