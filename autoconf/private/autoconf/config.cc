...
#include "autoconf/private/autoconf/check.h"
#include "autoconf/private/autoconf/json.h"
...
std::unique_ptr<Config> Config::from_file(
    ...
    if (!doc.contains("c_compiler") || !doc["c_compiler"].is_string()) {
        throw std::runtime_error("Missing or invalid 'c_compiler' field");
    }
    config->c_compiler = doc["c_compiler"].get<std::string>();

    if (!doc.contains("cpp_compiler") || !doc["cpp_compiler"].is_string()) {
        throw std::runtime_error("Missing or invalid 'cpp_compiler' field");
    }
    config->cpp_compiler = doc["cpp_compiler"].get<std::string>();

    // Parse compiler flags
    if (doc.contains("c_flags") && doc["c_flags"].is_array()) {
    ...
    if (doc.contains("cpp_flags") && doc["cpp_flags"].is_array()) {
    ...
    // Parse linker flags
    if (doc.contains("c_link_flags") && doc["c_link_flags"].is_array()) {
    ...
    if (doc.contains("cpp_link_flags") && doc["cpp_link_flags"].is_array()) {
    ...
    // Parse linker
    if (doc.contains("linker") && doc["linker"].is_string()) {
        config->linker = doc["linker"].get<std::string>();
    }

    // Parse compiler type
    if (doc.contains("compiler_type") && doc["compiler_type"].is_string()) {
        config->compiler_type = doc["compiler_type"].get<std::string>();
    }

    // Parse checks
    if (!doc.contains("checks") || !doc["checks"].is_array()) {
        throw std::runtime_error("Missing or invalid 'checks' field");
    }
    ...
    if (doc.contains("package_name") && doc["package_name"].is_string()) {
        config->package_name = doc["package_name"].get<std::string>();
    }

    if (doc.contains("package_version") && doc["package_version"].is_string()) {
        config->package_version = doc["package_version"].get<std::string>();
    }
