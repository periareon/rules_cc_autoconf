...
def create_package_info_file(ctx, package_name = "", package_version = ""):
    """Create a package info JSON file from package_name and package_version.
    ...
    Returns:
        A File representing the package info JSON file.
    """
    package_info_file = ctx.actions.declare_file("{}.pkg_info.json".format(
        ctx.label.name,
    ))
    ctx.actions.write(
...
def create_config_dict(toolchain_info, checks, package_info_path):
    ...
    Args:
        ...
        package_info_path (File): Path to package info JSON file.
