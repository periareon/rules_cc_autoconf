...
bool CheckRunner::try_compile(const std::string& code,
    ...
    std::filesystem::path source_file =
        tmp_dir / ("conftest" + get_file_extension(language));
    ...
    if (is_msvc) {
        ...
        cmd.push_back("/c");
        cmd.push_back("/Fo" + (tmp_dir / "conftest.obj").string());
        ...
        cmd.push_back(source_file.string());
        cmd.push_back("-o");
        cmd.push_back((tmp_dir / "conftest.o").string());
    }
...
std::optional<int> CheckRunner::try_compile_and_run(
    ...
    if (is_msvc) {
        // MSVC uses /c for compile-only and /Fo for object output
        cmd.push_back("/c");
        object_file = tmp_dir / "conftest.obj";
        ...
        cmd.push_back(source_file.string());
        cmd.push_back("-o");
        object_file = tmp_dir / "conftest.o";
        cmd.push_back(object_file.string());
    }
...
#ifdef _WIN32
    std::filesystem::path executable = tmp_dir / "conftest.exe";
#else
    std::filesystem::path executable = tmp_dir / "conftest";
#endif
...
bool CheckRunner::try_compile_and_link_with_lib(const std::string& code,
    ...
    if (is_msvc) {
        ...
        executable = tmp_dir / "conftest.exe";
