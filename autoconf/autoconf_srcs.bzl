...
def _autoconf_srcs_impl(ctx):
    ...
    results_files = ac_info.results.to_list()

    outputs = []
    args = ctx.actions.args()

    # Pass all individual results files directly to src_gen runner (it merges internally)
    for results_file in results_files:
        args.add("--results", results_file)

    inputs = results_files
    ...
    for src, define in ctx.attr.srcs.items():
        ...
        src_args.add("--src", "{}={}={}".format(in_file.path, define, out.path))

        ctx.actions.run(
            executable = ctx.executable._runner,
            arguments = [args, src_args],
            inputs = depset(inputs + [in_file]),
            outputs = [out],
...
In this setup `autoconf_srcs` reads the results from `:config` and generates a
wrapped copy of `foo_impl.c` whose contents are effectively:

```cc
#define HAVE_FOO 1      // when the check for HAVE_FOO succeeds
/* or: #undef HAVE_FOO  // when the check fails */
#ifdef HAVE_FOO
/* original foo_impl.c contents ... */
#endif
```
...
""",
    attrs = {
        "autoconf": attr.label(
            doc = "The `autoconf` target which provides defines.",
            mandatory = True,
